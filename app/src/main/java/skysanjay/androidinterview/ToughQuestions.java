...
public class ToughQuestions extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        cate_tv.setText("Tough Questions");
        ...
        index=0;
        questionTV.setText(tough_ques[index]);
        answersTV.setText(default_ans);
        tvCurrentIndex_xx.setText(String.valueOf(index+1));
        tvTotalLength_yy.setText("/"+ tough_ques.length);

        //tts object and listener initialization
        tts_object = new TextToSpeech(ToughQuestions.this, new TextToSpeech.OnInitListener() {
            @Override
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.backB:
                ...
                break;
            case R.id.answerB:
                answersTV.setText(tough_ans[index]);
                break;
            case R.id.forwardB:
                answersTV.setText(default_ans);
                index++;
            ...
            case R.id.soundB:
                if (result == TextToSpeech.LANG_NOT_SUPPORTED || result == TextToSpeech.LANG_MISSING_DATA)
                    ...
                    tts_object.speak(tough_ans[index], TextToSpeech.QUEUE_FLUSH,null);
                break;
            case R.id.muteB:
                if (tts_object != null)
                    tts_object.stop();
